...
#  RESETAR AS LISTAS E ACESSAR A PRIMEIRA PÁGINA
for uf in range(5):
    lista_preco = []
    lista_marca = []
    lista_nome = []
    ...
    # PRINT DO NÚMERO DE PÁGINAS DO PRIMEIRO LINK
    numero_paginas = int(num.get_text().split()[-1])
    print(numero_paginas)

    # ACESSAR O PRIMEIRO OL DA PÁGINA, CADA OL ARMAZENA 3 ANÚNCIOS
    for i in range(1, numero_paginas + 1):
        anuncios = soup.find('section', {'class': 'ui-search-results'})
        grupos = anuncios.findAll('ol', class_='ui-search-layout ui-search-layout--grid')
        # PEGAR A DIV DO ANÚNCIO DA PÁGINA
        for dupla in grupos:
            carros = dupla.findAll('div', {'class':'ui-search-result__content-wrapper'})

            # PEGAR OS ELEMENTOS DA PÁGINAS
            for carro in carros:
                ...
                # PRINT PARA TESTE O QUE ESTÁ ARMAZENANDO NA LISTA
        ...
        # PRIMEIRO CONTADOR É DO NÚMERO DE CARROS E O SEGUNDO NO NÚMERO DE CARRO POR ESTADO
        print(f'Números de carros: {index_carros}  e cont: {cont}' )

        # IDENTIFICADOR DO INDEX, PARA MUDAR DE PÁGINA
        num = lista_urls[uf].find('_YearRange')

        # CONCATENA O LINK E INSERE O VALOR DO INDEX PARA MUDAR DE PÁGINA
